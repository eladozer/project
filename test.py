...
def handle_reg(dat):
    db.register(dat[1], dat[2], dat[3], dat[4], dat[5])
            ...
            if data[0].lower() == "register":
                ...
                if answer is True:
                    ...
                    if check_dates(date, date_object):
                        ...
                        data = db.get_params(data[1], True)
                    else:
                        data = db.get_params(data[1], False)
                ...
                name = data[1]
                new_name = data[2]
                db.update_name(name, new_name)
                await websocket.send("Name Updated")
...
async def main():
    print("Server Start")
    async with websockets.serve(handle_client, "10.0.0.21", 8820):
